...
impl<'a> Board<'a> {
	...
	for position in 0..15 {
	    ...
	    self.search_stack.push((visited_mask, position as u8, search));
	}
    ...
    #[inline(always)]
    fn traverse_new_path(&mut self, visited_mask: &u16, x: &u8, y: &u8, search: &IncSearch<'a, Letter, ()>){
	...
	if postfixes_exist && new_mask != Self::VISITED_BOARD {
	    self.search_stack.push((new_mask, candidate_position, new_search));	    
	}
    }
